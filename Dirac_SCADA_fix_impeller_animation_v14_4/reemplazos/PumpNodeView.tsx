    ...
    return (
      ...
      >
        <text
          ...
          textAnchor="start"
          fill="#1e293b"
          style={{ fontSize: 14, fontWeight: 850, pointerEvents: "none" }}
        >
          {name}
        </text>
        ...
        <circle
          ...
          fill="#f8fafc"
          stroke={motorStroke}
          strokeWidth={tapSelected ? 3.6 : 2.4}
          style={{ pointerEvents: "none" }}
        />

        {/* IMPORTANTE:
            La traslación al centro queda en el grupo exterior.
            La animación rota SOLAMENTE el grupo interior.
            Así animateTransform no reemplaza translate(43 5). */}
  ...
  return (
    ...
    >
      ...
      <circle
        ...
        fill="#f8fafc"
        stroke={motorStroke}
        strokeWidth={tapSelected ? 3.6 : 2.3}
        style={{ pointerEvents: "none" }}
      />

      {/* La traslación queda fija y la rotación ocurre adentro. */}
      <g transform="translate(0 20)" style={{ pointerEvents: "none" }}>
        <g>
          {[0, 90, 180, 270].map((deg) => (
            <path
              key={deg}
              d="M 0 -7.5 C 4.2 -7 6.2 -3.3 3.0 -0.8 L 0 0 Z"
              transform={`rotate(${deg})`}
              fill={running && online ? "#0ea5e9" : "#94a3b8"}
            />
          ))}
