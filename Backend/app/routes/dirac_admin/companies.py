...
def list_companies(user=Depends(require_user)):
    with get_conn() as conn, conn.cursor(row_factory=dict_row) as cur:
        # Debe ser owner/admin en alguna empresa
...
def patch_company(company_id: int, payload: CompanyPatch, user=Depends(require_user)):
    with get_conn() as conn, conn.cursor(row_factory=dict_row) as cur:
        assert_is_admin_in_company(cur, user["user_id"], company_id)
        try:
            cur.execute(
                ...
                "WHERE id=%s RETURNING id, name, status, legal_name, cuit",
...
@router.delete("/companies/{company_id}/users/{target_user_id}", summary="Quitar usuario de la empresa (admin)")
