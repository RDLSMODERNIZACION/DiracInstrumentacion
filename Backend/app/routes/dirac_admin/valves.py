...
def list_valves(
    ...
    - SQL armado dinámicamente (sin (param IS NULL)) -> evita AmbiguousParameter.
    """
...
def update_valve(valve_id: int, payload: ValvePatch, user=Depends(require_user)):
    ...
    with get_conn() as conn, conn.cursor(row_factory=dict_row) as cur:
        ...
        cur.execute(
            """
            SELECT v.id, v.name, v.location_id, l.company_id
            FROM valves v
            LEFT JOIN locations l ON l.id = v.location_id
            WHERE v.id=%s
            """,
        ...
        try:
            if new_name is None and loc_id is None and new_kind is None:
                ...
                return {
                    ...
                    "kind": new_kind or current.get("kind"),
                }
...
def delete_valve(valve_id: int, user=Depends(require_user)):
    with get_conn() as conn, conn.cursor(row_factory=dict_row) as cur:
        ...
        try:
            cur.execute("DELETE FROM valves WHERE id=%s", (valve_id,))
            if cur.rowcount == 0:
                raise HTTPException(404, "Válvula no encontrada")
            conn.commit()
            return  # 204
            ...
            conn.rollback()
            raise HTTPException(400, f"Delete valve error: {e}")
