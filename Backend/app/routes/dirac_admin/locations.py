...
def list_locations(company_id: int | None = Query(default=None), user=Depends(require_user)):
    with get_conn() as conn, conn.cursor(row_factory=dict_row) as cur:
        cur.execute("SELECT EXISTS(SELECT 1 FROM company_users WHERE user_id=%s AND role IN ('owner','admin')) AS ok",
                    (user["user_id"],))
        if not cur.fetchone()["ok"]:
            raise HTTPException(403, "Requiere owner/admin")
        if company_id:
            cur.execute("SELECT id, name, company_id, address, lat, lon FROM locations WHERE company_id=%s ORDER BY name",
                        (company_id,))
        else:
...
def location_stats(location_id: int, user=Depends(require_user)):
    with get_conn() as conn, conn.cursor(row_factory=dict_row) as cur:
        ...
        cur.execute("SELECT COUNT(*) AS n FROM tanks  WHERE location_id=%s", (location_id,)); tk = cur.fetchone()["n"]
        cur.execute("SELECT COUNT(*) AS n FROM pumps  WHERE location_id=%s", (location_id,)); pu = cur.fetchone()["n"]
        cur.execute("SELECT COUNT(*) AS n FROM valves WHERE location_id=%s", (location_id,)); va = cur.fetchone()["n"]
        return {"location_id": location_id, "company_id": loc["company_id"], "name": loc["name"],
...
def patch_location(
    ...
    with get_conn() as conn, conn.cursor(row_factory=dict_row) as cur:
        ...
        try:
            cur.execute(
                "UPDATE locations SET name=COALESCE(%s,name), address=COALESCE(%s,address), "
            ...
            conn.rollback(); raise HTTPException(400, f"Update location error: {e}")
...
def delete_location(location_id: int, move_to: int | None = Query(default=None), user=Depends(require_user)):
    with get_conn() as conn, conn.cursor(row_factory=dict_row) as cur:
        ...
        if move_to is not None:
            ...
            conn.commit(); return {"ok": True, "moved_to": move_to, "deleted": location_id}
        if total > 0:
            raise HTTPException(409, {"message": "La localización tiene activos asignados",
                                      "counts": {"tanks": tk, "pumps": pu, "valves": va}})
        cur.execute("DELETE FROM locations WHERE id=%s", (location_id,))
        conn.commit(); return {"ok": True, "deleted": location_id}
