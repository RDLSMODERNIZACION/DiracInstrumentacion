# app/routes/dirac_admin/manifolds.py
...
router = APIRouter(prefix="/dirac/admin", tags=["admin-manifolds"])

# -----------------------------
# Modelos
# -----------------------------
class ManifoldCreate(BaseModel):
    name: str
...
class ManifoldPatch(BaseModel):
    ...
    @field_validator("name")
    @classmethod
    def _trim_opt(cls, v: Optional[str]) -> Optional[str]:
        return (v or "").strip() if v is not None else v

# -----------------------------
# Helpers permisos / utilitarios
# -----------------------------
...
def _manifold_company_id(cur, manifold_id: int) -> Optional[int]:
    cur.execute(
        """
        SELECT l.company_id
        FROM manifolds m
...
def _count_refs_manifold(cur, manifold_id: int) -> Dict[str, int]:
    """Cuenta referencias que bloquean borrado."""
    node_id = f"manifold:{manifold_id}"
    cur.execute("SELECT COUNT(*) AS c FROM public.layout_manifolds WHERE manifold_id=%s", (manifold_id,))
    layout = int(cur.fetchone()["c"])
    cur.execute("SELECT COUNT(*) AS c FROM public.layout_edges WHERE src_node_id=%s OR dst_node_id=%s", (node_id, node_id))
    edges = int(cur.fetchone()["c"])
    return {"layout": layout, "edges": edges}

def _seed_layout_manifold(cur, manifold_id: int):
    """Crea fila de layout si no existe (0,0) para que aparezca en el diagrama."""
    cur.execute(
        """
        INSERT INTO public.layout_manifolds (manifold_id, node_id, x, y)
        VALUES (%s, %s, 0, 0)
        ON CONFLICT (node_id) DO NOTHING
        """,
        (manifold_id, f"manifold:{manifold_id}"),
    )

# -----------------------------
# GET: listar (viewer-safe) con filtros
# -----------------------------
@router.get("/manifolds", summary="Listar manifolds (con filtros por empresa/ubicación)")
def list_manifolds(
    ...
    with get_conn() as conn, conn.cursor(row_factory=dict_row) as cur:
        ...
        JOIN v_user_locations v
          ON v.location_id = m.location_id
         AND v.user_id = %s
        """
...
# -----------------------------
@router.get("/manifolds/{manifold_id}/refs", summary="Contar referencias de manifold")
def manifold_refs(manifold_id: int, user=Depends(require_user)):
    with get_conn() as conn, conn.cursor(row_factory=dict_row) as cur:
        company_id = _manifold_company_id(cur, manifold_id)
        if company_id is None:
            raise HTTPException(404, "Manifold no encontrado")
        _assert_admin_company(cur, user, company_id)
        return _count_refs_manifold(cur, manifold_id)

# -----------------------------
# POST: crear (owner/admin)
# -----------------------------
@router.post("/manifolds", summary="Crear manifold (owner/admin)", status_code=201)
def create_manifold(payload: ManifoldCreate, user=Depends(require_user)):
    with get_conn() as conn, conn.cursor(row_factory=dict_row) as cur:
        # Empresa objetivo
        ...
        try:
            cur.execute(
                """
                INSERT INTO manifolds (name, location_id)
                VALUES (%s, %s)
                RETURNING id, name, location_id
                """,
                ((payload.name or "").strip(), loc_id),
            )
            row = cur.fetchone()
            _seed_layout_manifold(cur, row["id"])  # semilla layout
            conn.commit()
            return row
        except Exception as e:
            conn.rollback()
            raise HTTPException(400, f"Create manifold error: {e}")

# -----------------------------
# PATCH: actualizar (owner/admin)
# -----------------------------
@router.patch("/manifolds/{manifold_id}", summary="Actualizar manifold (owner/admin)")
def update_manifold(manifold_id: int, payload: ManifoldPatch, user=Depends(require_user)):
    with get_conn() as conn, conn.cursor(row_factory=dict_row) as cur:
        # Estado actual
        cur.execute(
            """
            SELECT m.id, m.name, m.location_id, l.company_id
            FROM manifolds m
            LEFT JOIN locations l ON l.id = m.location_id
            WHERE m.id=%s
            """,
        ...
        try:
            ...
            conn.rollback()
            raise HTTPException(400, f"Update manifold error: {e}")

# -----------------------------
# DELETE: eliminar (owner/admin) con auto-cascada si es SOLO layout
# -----------------------------
@router.delete("/manifolds/{manifold_id}", summary="Eliminar manifold (owner/admin)", status_code=204)
def delete_manifold(manifold_id: int, force: bool = Query(False), user=Depends(require_user)):
    """
    - Si solo existe layout => se borra sin pedir ?force.
    - Si hay edges además del layout y no se pasa ?force => 409 + counts.
    ...
    with get_conn() as conn, conn.cursor(row_factory=dict_row) as cur:
        ...
        counts = _count_refs_manifold(cur, manifold_id)
        ...
        if not force and not layout_only and any(v > 0 for v in counts.values()):
            raise HTTPException(409, {"message": "El manifold tiene referencias. Reintenta con ?force=true", "counts": counts})

        node_id = f"manifold:{manifold_id}"
        try:
            cur.execute("DELETE FROM public.layout_edges WHERE src_node_id=%s OR dst_node_id=%s", (node_id, node_id))
            cur.execute("DELETE FROM public.layout_manifolds WHERE manifold_id=%s OR node_id=%s", (manifold_id, node_id))
            cur.execute("DELETE FROM public.manifolds WHERE id=%s", (manifold_id,))
            if cur.rowcount == 0:
                raise HTTPException(404, "Manifold no encontrado")
            conn.commit()
            ...
            conn.rollback(); raise HTTPException(400, f"Delete manifold error: {e}")
