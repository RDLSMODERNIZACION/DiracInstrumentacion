...
def change_user_password(
    user_id: int,
    new_password: str = Query(..., min_length=4, max_length=128),
    user=Depends(require_user),
):
    with get_conn() as conn, conn.cursor(row_factory=dict_row) as cur:
        if not user.get("superadmin"):
            cur.execute(
                ...
                SELECT EXISTS(
                  ...
                  JOIN company_users cu_target
                    ON cu_target.user_id = %s
                   AND cu_target.company_id = cu_admin.company_id
                 WHERE cu_admin.user_id = %s
            ...
            if not cur.fetchone()["ok"]:
                raise HTTPException(403, "Requiere owner/admin en alguna empresa compartida (o superadmin)")

        cur.execute(
            "UPDATE app_users SET password_plain=%s, password_updated_at=now() WHERE id=%s RETURNING id",
