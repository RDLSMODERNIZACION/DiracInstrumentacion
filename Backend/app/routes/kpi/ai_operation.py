...
def _normalize_analysis(obj: Dict[str, Any]) -> Dict[str, Any]:
    ...
    return {
        "estado": estado,
        "titulo": str(obj.get("titulo") or "AnÃ¡lisis operativo de la red").strip(),
...
def _call_openai(context: Dict[str, Any]) -> Dict[str, Any]:
    ...
    instructions = """
Sos un asistente de operaciÃ³n para una red municipal de agua.
AnalizÃ¡ exclusivamente los datos JSON provistos. No inventes mediciones, causas ni equipos.

Objetivo:
- interpretar el estado hidrÃ¡ulico, elÃ©ctrico y de disponibilidad;
...
- seÃ±alar inconsistencias que ameriten revisiÃ³n;
- proponer acciones de VERIFICACIÃ“N u OPERACIÃ“N, pero nunca afirmar que ejecutaste una maniobra.

Reglas:
- Si no hay evidencia suficiente para una causa, decÃ­ que debe verificarse.
- Un FP bajo con carga significativa merece atenciÃ³n.
- Una caÃ­da rÃ¡pida de almacenamiento merece atenciÃ³n aunque ningÃºn tanque estÃ© crÃ­tico.
- No confundas "online" con "disponible".
- No supongas potencia individual de una bomba si expected_power_kw estÃ¡ vacÃ­o.
- No uses conocimiento externo para completar datos faltantes.
- Las recomendaciones deben ser concretas y cortas.
- RespondÃ© en espaÃ±ol tÃ©cnico claro.

DevolvÃ© SOLO JSON vÃ¡lido con esta forma:
{
...
"""
    ...
    if not res.ok:
        ...
        raise HTTPException(
            status_code=502,
            detail=f"OpenAI respondiÃ³ {res.status_code}: {detail}",
        )

    raw = res.json()
    text = _extract_output_text(raw).strip()
    if not text:
        raise HTTPException(status_code=502, detail="OpenAI no devolviÃ³ texto utilizable.")
    ...
    try:
        ...
        raise HTTPException(
            status_code=502,
            detail="La respuesta de IA no vino como JSON vÃ¡lido.",
        )
...
@router.get("/latest", summary="Ãšltimo anÃ¡lisis IA guardado")
def ai_latest():
    with get_conn() as conn, conn.cursor(row_factory=dict_row) as cur:
        row = _fetch_one(
            cur,
...
@router.post("/analyze", summary="Analiza la operaciÃ³n actual con IA")
