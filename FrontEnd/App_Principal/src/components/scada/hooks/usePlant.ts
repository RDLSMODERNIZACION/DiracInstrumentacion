...
type Tank = {
  ...
  levelPct?: number|null; age_sec?: number|null; ageSec?: number|null; online?: boolean|null;
  alarm?: "normal"|"alerta"|"critico";
  latest?: any; thresholds?: Thresholds;
};
type Pump = {
  id: number; name: string; state?: "run"|"stop";
  location_id?: number|null; location_name?: string|null;
  locationId?: number|null;  locationName?: string|null;
  location?: { id?: number|null; name?: string|null };
  age_sec?: number|null; ageSec?: number|null; online?: boolean|null;
  latest_event_id?: number|null; event_ts?: string|null; latest_hb_id?: number|null; hb_ts?: string|null;
  latest?: any;
};
...
async function getJSON(path: string) {
  ...
  const res = await fetch(url.toString(), {
    method: "GET",
    headers: { Accept: "application/json", "Cache-Control": "no-cache", ...authHeaders() },
    cache: "no-store",
  });
...
function toNumOr(def: number, x: any) { const n = Number(x); return Number.isFinite(n) ? n : def; }
...
function mapTanks(rows: any[]): Tank[] {
  return rows.map((r) => {
    ...
    return {
      id, name,
      location_id, location_name, online, levelPct, alarm,
      locationId: location_id, locationName: location_name, location: { id: location_id, name: location_name },
      ageSec: age_sec, age_sec,
      thresholds: {
...
function mapPumps(rows: any[]): Pump[] {
  return rows.map((r) => {
    ...
    return {
      ...
      ...(age_sec !== undefined ? { age_sec } : {}), ...(online !== undefined ? { online } : {}),
    };
  });
}
...
export function usePlant(pollMs = 1000, allowedLocationIds?: Set<number>): UsePlant {
  ...
  React.useEffect(() => { plantRef.current = plant; }, [plant]);
  ...
  const fetchAll = React.useCallback(async () => {
    ...
    try {
      ...
      const [tanksRes, pumpsRes] = await Promise.allSettled([
        getFirstJSON(["/tanks/config", "/dirac/admin/tanks/config"]),
        getFirstJSON(["/pumps/config", "/dirac/admin/pumps/config"]),
      ]);
  ...
  React.useEffect(() => {
    ...
    const start = () => { if (pollMs > 0 && timer == null) timer = window.setInterval(fetchAll, pollMs); };
