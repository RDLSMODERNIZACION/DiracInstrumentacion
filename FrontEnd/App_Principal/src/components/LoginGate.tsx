...
function hasAuth() {
  return !!(localStorage.getItem("dirac_basic") || sessionStorage.getItem("dirac_basic"));
}
...
export default function LoginGate({ children }: { children: React.ReactNode }) {
  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      // validá contra /dirac/me
      await getJSON("/dirac/me");
      setOk(true);
    } catch (e: any) {
  ...
  return (
    <div className="min-h-screen grid place-items-center bg-slate-50 p-4">
      <form onSubmit={onSubmit} className="bg-white border rounded p-4 w-full max-w-sm space-y-3">
        ...
        <div>
          ...
          <input className="border rounded px-2 py-1 w-full" type="password" value={password} onChange={e=>setPassword(e.target.value)} />
        </div>
        ...
        <button type="submit" disabled={checking} className="px-3 py-1.5 rounded bg-slate-900 text-white w-full">
