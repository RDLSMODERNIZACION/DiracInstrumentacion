...
const products = [
  { title: "Farola solar 400W", image: "/img/productos/luminaria-solar-400w.png" },
  { title: "Farola solar All in One", image: "/img/productos/luminaria-solar-all-in-one.png" },
  { title: "Panel solar de alto rendimiento", image: "/img/productos/panel-solar-alto-rendimiento.png" },
];
  ...
  return (
    <div className="dl">
      <style>{`
        ...
        .dl-hero{position:relative;height:min(760px,calc(100svh - 76px));min-height:630px;background:var(--navy);overflow:hidden;color:white;user-select:none}.dl-slide{position:absolute;inset:0;opacity:0;transform:scale(1.045);transition:opacity 1.2s ease,transform 7s ease;pointer-events:none}.dl-slide.active{opacity:1;transform:scale(1)}.dl-slide img{width:100%;height:100%;object-fit:cover;display:block;filter:saturate(.9) contrast(1.03)}.dl-slide::after{content:"";position:absolute;inset:0;background:linear-gradient(90deg,rgba(3,13,24,.96) 0%,rgba(3,13,24,.84) 38%,rgba(3,13,24,.45) 68%,rgba(3,13,24,.22) 100%),linear-gradient(0deg,rgba(3,13,24,.5),transparent 48%)}
        ...
        .dl-side-arrow{position:absolute;z-index:8;top:50%;transform:translateY(-50%);width:58px;height:58px;display:grid;place-items:center;border-radius:999px;border:1px solid rgba(255,255,255,.28);background:rgba(3,13,24,.42);backdrop-filter:blur(12px);color:#fff;cursor:pointer;transition:.2s ease;box-shadow:0 14px 36px rgba(0,0,0,.18)}.dl-side-arrow:hover{background:rgba(3,13,24,.68);border-color:rgba(255,255,255,.48);transform:translateY(-50%) scale(1.04)}.dl-side-arrow.left{left:28px}.dl-side-arrow.right{right:28px}.dl-side-arrow:active{transform:translateY(-50%) scale(.96)}
        .dl-hero-bottom{position:absolute;left:0;right:0;bottom:0;z-index:6;border-top:1px solid rgba(255,255,255,.14);background:rgba(2,12,22,.42);backdrop-filter:blur(12px)}.dl-hero-control{min-height:76px;display:flex;align-items:center;justify-content:space-between;gap:20px}.dl-dots{display:flex;align-items:center;gap:9px}.dl-dot{width:38px;height:3px;border:0;padding:0;background:rgba(255,255,255,.25);cursor:pointer;overflow:hidden;border-radius:99px}.dl-dot.active::after{content:"";display:block;width:100%;height:100%;background:var(--green);transform-origin:left;animation:dlProgress 5.6s linear forwards}.dl-hero.paused .dl-dot.active::after{animation-play-state:paused}@keyframes dlProgress{from{transform:scaleX(0)}to{transform:scaleX(1)}}.dl-slide-info{display:flex;align-items:center;gap:14px;color:rgba(255,255,255,.72);font-size:12px;font-weight:800;text-transform:uppercase;letter-spacing:.1em}
      ...
      <nav className="dl-nav">
        ...
        {mobileOpen && <div className="dl-mobile"><a href="#equipo" onClick={() => setMobileOpen(false)}>Nuestro equipo</a><a href="#productos" onClick={() => setMobileOpen(false)}>Catálogo</a><a href="#empresa" onClick={() => setMobileOpen(false)}>Empresa</a><a href="#contacto" onClick={() => setMobileOpen(false)}>Contacto</a><Link className="dl-login" to="/login">Ingresar al panel</Link></div>}
      </nav>

      <main id="inicio">
        <section
          className={`dl-hero ${isHeroPaused ? "paused" : ""}`}
          onPointerDown={() => setIsHeroPaused(true)}
          onPointerUp={() => setIsHeroPaused(false)}
          onPointerCancel={() => setIsHeroPaused(false)}
          onPointerLeave={() => setIsHeroPaused(false)}
        >
          {heroSlides.map((item,index)=><div className={`dl-slide ${index===slide?"active":""}`} key={item.title}><img src={item.image} alt="" /></div>)}

          <button className="dl-side-arrow left" onClick={()=>goToSlide(slide-1)} aria-label="Anterior"><ChevronLeft size={28}/></button>
          <button className="dl-side-arrow right" onClick={()=>goToSlide(slide+1)} aria-label="Siguiente"><ChevronRight size={28}/></button>
          ...
          <div className="dl-shell dl-hero-grid">
            <div className="dl-hero-copy" key={slide}>
              ...
              <div className="dl-actions"><a className="dl-primary" href="#equipo">Nuestro equipo <ArrowRight size={17}/></a><a className="dl-secondary" href="#productos">Ver catálogo</a></div>
            </div>
          </div>
        ...
        <section className="dl-section paper" id="equipo"><div className="dl-shell"><div className="dl-section-head"><div><div className="dl-kicker">Nuestro equipo</div><h2>Movilidad preparada para operación en campo.</h2></div><p>Contamos con unidades propias para montaje, mantenimiento y asistencia técnica, habilitadas para trabajos en campos petroleros y operación en yacimientos.</p></div><div className="dl-project-grid">{teamVehicles.map((vehicle)=><article className="dl-project" key={vehicle.title}><img src={vehicle.image} alt={vehicle.title}/><div className="dl-project-copy"><small>{vehicle.category}</small><h3>{vehicle.title}</h3><p>{vehicle.text}</p></div></article>)}</div></div></section>

        <section className="dl-section white" id="productos"><div className="dl-shell"><div className="dl-section-head"><div><div className="dl-kicker">Catálogo comercial</div><h2>Productos para venta directa.</h2></div><p>Farolas solares y paneles fotovoltaicos listos para provisión directa. Mostramos únicamente los productos comerciales disponibles.</p></div><div className="dl-product-grid">{products.map((product)=><article className="dl-product" key={product.title}><div className="dl-product-media catalog-clean"><img src={product.image} alt={product.title}/></div></article>)}</div></div></section>

        <section className="dl-proof" id="empresa"><div className="dl-shell dl-proof-grid"><div><div className="dl-eyebrow">DIRAC Servicios Energía</div><h2>Capacidad técnica, presencia en campo y soluciones integrales.</h2><p>Empresa de Rincón de los Sauces con más de 10 años de experiencia en ingeniería, automatización, instrumentación y eficiencia energética.</p></div><div className="dl-proof-list"><div className="dl-proof-item"><ShieldCheck size={24}/><strong>Profesionales matriculados</strong><span>Ingeniería y ejecución con respaldo técnico.</span></div><div className="dl-proof-item"><Zap size={24}/><strong>Soluciones integrales</strong><span>Desde el relevamiento hasta la puesta en servicio.</span></div><div className="dl-proof-item"><SunMedium size={24}/><strong>Energía aplicada</strong><span>Sistemas solares y eficiencia energética.</span></div><div className="dl-proof-item"><Camera size={24}/><strong>Tecnología de campo</strong><span>Monitoreo, automatización y activos remotos.</span></div></div></div></section>

        <section className="dl-cta" id="contacto"><div className="dl-shell dl-cta-inner"><div><h2>¿Necesitás cotizar un producto o proyecto?</h2><p>Contanos qué necesitás y te ayudamos a definir la solución.</p></div><div className="dl-contact"><a href="tel:+542994292985"><Phone size={16}/> 299 4292985</a><a href="tel:+542993251398"><Phone size={16}/> 299 3251398</a><a href="mailto:administracion@diracserviciosenergia.com"><Mail size={16}/> Escribir por email</a></div></div></section>
      </main>
