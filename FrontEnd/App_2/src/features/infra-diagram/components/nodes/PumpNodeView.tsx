...
export default function PumpNodeView({
  n, getPos, setPos, onDragEnd, showTip, hideTip,
  enabled = true, onClick, tapSelected = false,
}: {
  n: PumpNode & { name?: string | null; in_maintenance?: boolean | null; orientacion?: PumpOrientation | null; meta?: any; orientation?: PumpOrientation };
  getPos: any; setPos: any; onDragEnd: () => void;
  showTip: (e: React.MouseEvent, content: { title: string; lines: string[] }) => void;
  hideTip: () => void; enabled?: boolean; onClick?: () => void; tapSelected?: boolean;
}) {
  const drag = useNodeDragCommon(n, getPos, setPos, onDragEnd, hideTip, enabled);
  const orientation: PumpOrientation = (n.orientacion as PumpOrientation) || ((n as any).meta?.orientation as PumpOrientation) || ((n as any).orientation as PumpOrientation) || "vertical";
  ...
  const tipLines = [`ID: ${n.id}`, `Estado: ${running ? "En marcha" : "Detenida"}`, `Comunicación: ${online ? "Online" : "Offline"}`, `Montaje: ${orientation === "horizontal" ? "Horizontal" : "Vertical"}`];
  ...
  if (orientation === "horizontal") {
    return <g transform={`translate(${n.x}, ${n.y})`} onPointerDown={(e) => {
      ...
      <rect x={-48} y={-16} width={70} height={34} rx={12} fill={motorFill} stroke={motorStroke} strokeWidth={tapSelected?3.6:2.3}/>
      ...
      <circle cx={43} cy={5} r={18} fill="#f8fafc" stroke={motorStroke} strokeWidth={tapSelected?3.2:2.4}/>
      <g transform="translate(43 5)">{[0,90,180,270].map((deg)=><path key={deg} d="M 0 -11 C 6 -10 9 -5 5 -1 L 0 0 Z" transform={`rotate(${deg})`} fill={running&&online?"#0ea5e9":"#94a3b8"}/>)}<circle r={3} fill={running&&online?"#0284c7":"#64748b"}/>{running&&online&&<animateTransform attributeName="transform" type="rotate" from="0 0 0" to="360 0 0" dur="0.9s" repeatCount="indefinite"/>}</g>
      <rect x={-42} y={25} width={94} height={10} rx={3} fill="#e2e8f0" stroke="#64748b" strokeWidth={1.5}/>
      <rect x={-23} y={41} width={46} height={16} rx={8} fill={statusFill}/><text x={0} y={53} textAnchor="middle" fill="#fff" style={{fontSize:10,fontWeight:900,pointerEvents:"none"}}>{statusText}</text>
    </g>;
  }
  ...
  return <g transform={`translate(${n.x}, ${n.y})`} onPointerDown={(e) => {
    ...
    <rect x={-18} y={-36} width={36} height={48} rx={10} fill={motorFill} stroke={motorStroke} strokeWidth={tapSelected?3.6:2.2}/>
    <path d="M -18 -28 Q 0 -45 18 -28 L 18 -21 L -18 -21 Z" fill="#dbe3ea" stroke="#64748b" strokeWidth={1.5}/>
    <circle cx={0} cy={20} r={17} fill="#f8fafc" stroke={motorStroke} strokeWidth={tapSelected?3.2:2.3}/>
    <g transform="translate(0 20)">{[0,90,180,270].map((deg)=><path key={deg} d="M 0 -10 C 5 -9 8 -4 4 -1 L 0 0 Z" transform={`rotate(${deg})`} fill={running&&online?"#0ea5e9":"#94a3b8"}/>)}<circle r={3} fill={running&&online?"#0284c7":"#64748b"}/>{running&&online&&<animateTransform attributeName="transform" type="rotate" from="0 0 0" to="360 0 0" dur="0.9s" repeatCount="indefinite"/>}</g>
